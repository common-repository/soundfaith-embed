 ...
 *
 * @wordpress-plugin
 * Plugin Name:       Sermon Embed
 * Plugin URI:        https://github.com/eriktdesign/soundfaith-embed/
 * Description:       Enables Faithlife Sermons as an embed provider
 * Version:           1.2.2
...
class SF_Embed {
	...
	public $embed_base = "https://sermons.faithlife.com/embed/";
	...
	protected $sermon_regex = '/(?:soundfaith|sermons\.faithlife)\.com\/(sermons)\/(\d+)/i';
	protected $profile_regex = '/(?:soundfaith|sermons\.faithlife)\.com\/(profile)\/(.+)/i';
	protected $series_regex = '/(?:soundfaith|sermons\.faithlife)\.com\/(series)\/(\d+)/i';
	...
	public function add_providers() {
		...
		wp_embed_register_handler( 'sf_series', $this->series_regex, array( $this, 'sf_embed' ) );
	}
	...
	public function get_dimensions( $type ) {
		...
		} elseif( "profile" == $type || "series" == $type ) {
	...
	public function get_embed_url( $type, $id ) {
		...
		if( "profile" == $type ) {
			...
			// Use the same params for profiles on series
			return add_query_arg( $this->profile_options, $url );
		} else {
